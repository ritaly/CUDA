...
__global__ void addKernel(int *c, const int *a, const int *b) // funkcja uruchamiana na karcie graficnzej  
{
	...
	//c[i] = blockDim.x * j + i;
	c[blockIdx.x*blockDim.x + threadIdx.x] = 1000 * j + i;
...
cudaError_t addWithCuda(int *c, const int *a, const int *b, unsigned int size)
{
	...
	addKernel << < dimGrid, dimBlock >> >(dev_c, dev_a, dev_b); // jaka struktura blokow - 1: blok jest 1, size u nas = 5 czyli tworzymy blok 5 watkowy i pozostale 27 bedzie nic nei robilo 
